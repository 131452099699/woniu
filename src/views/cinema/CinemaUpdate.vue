...
export default {
  data() {
    return {
      cinemaTags: [], //电影院标签
      form: {
        id: this.$route.params.id, //从路由中获取电影id
        cinema_name: "",
        address: "",
        province: "",
        city: "",
      ...
      rules: {
        ...
        provice: [
          { required: true, message: "请在地图中选择", trigger: "blur" },
        ],
        city: [
          { required: true, message: "请在地图中选择地址", trigger: "blur" },
        ],
          ...
          map.on("click", (e) => {
            ...
            geocoder.getAddress([lnglat.KL, lnglat.kT], (status, result) => {
              ...
              if (status == "complete") {
                ...
                this.$message.error("地图信息加载失败，请重试");
              }
            });
          });
        })
