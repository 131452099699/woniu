...
router.post('/movie-info/add',(req,resp)=>{
    let {categoryld,cover,title,type,starActor,showingon,score,description,duration}=req.body
    // 表单验证
    let schema=Joi.object({
        categoryld:Joi.number().required(), 
    ...
    pool.query(sql,[categoryld,cover,title,type,starActor,showingon,score,description,duration],(error,result)=>{
        if(error){
...
router.get('/movie-infos',async (req,resp)=>{
    ...
    // 执行查询数组业务
    try {
        let startIndex=(page-1)*pagesize //下标
...
router.post('/movie-info/bind-actors',async(req,resp)=>{
    ...
    try {
        ...
        pool.querySync(sql1,[movie_id])   
        if(!actor_ids){
            resp.send(Response.ok())
            return;
        }  
        // 执行sql，将当前movie_id与actor_ids绑定在一起,全部插入数据库
        let params=''
        let paramsArray=[]
