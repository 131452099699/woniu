...
class Seat{
    constructor(x,y){
        this.x=x
        this.y=y
        this.type=0 //座位为  0:未分配 1：普通座位 2：过道
...
class SeatSelector{
        ...
        for(let i=0;i<this.rows;i++){ 
            for(let j=0;j<this.cols;j++){
                ...
                if(seat.type==1){
                    ...
                    this.cvs.fillRect(
                        j*this.seatSize+(j+1)*this.margin,i*this.seatSize+(i+1)*this.margin,this.seatSize,
                        this.seatSize
                        );
                    continue;
                }
                ...
                if(seat.selected){
                    ...
                    this.cvs.fillRect(j*this.seatSize+(j+1)*this.margin,i*this.seatSize+(i+1)*this.margin,this.seatSize,
                    this.seatSize)
                }
    ...
    getSeatTemplateJsonString(){
        ...
        for(let i=0;i<this.rows;i++){
          ...
          let filterRowData = rowData.filter((item)=>{
                return item.type!=2
            })
