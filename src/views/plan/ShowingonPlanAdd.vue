  ...
  data() {
    return {
      ...
      rules: {
        ...
        cinema_room_id: [
          { required: true, message: "请填写电影放映厅ID", trigger: "blur" },
        ],
        movie_id: [
      ...
      this.$refs["form"].validate((valid) => {
        if (valid) {
          this.$http.ShowingonPlanApi.add(this.form).then((res) => {
            console.log("验证表单成功后,发送请求", res);
            // 获取电影名称
            let movie_name = this.$refs.movieSelector.selectedLabel;
            if (res.data.code == 200) {
              this.$notify({
                title: "添加排片计划成功",
                message: `${this.cinemaRoomInfo.cinema_name}影院的${this.cinemaRoomInfo.cinema_room_name}将会在${this.form.showingon_date}${this.form.showingon_time}放映《${movie_name}》`,
                type: "success",
              });
              this.$router.go(-1);
            }
